...
I would like to build my own air-gapped, cold Ethereum wallet using
FOSS wallet software, the Tails OS and a couple of thumb drives. 

I've done this in the past for Bitcoin using Electrum as my wallet
software. I'd like to find similar software in the Ethereum ecosystem
since Electrum only works with Bitcoin.

* Quick Comparison Table

| Name                   | Open Source? | Runs In Tails?     | Runs In Debian? | Signs Trx On An Air-Gapped System?  | Requires Full Sync? | Seed Password? | Notes                                                                               |
|------------------------+--------------+--------------------+-----------------+-------------------------------------+---------------------+----------------+-------------------------------------------------------------------------------------|
...
| [[#MyEthereumWallet][MyEthereumWallet (MEW)]] | Yes          | Only in 3.2 [fn:1] | Yes             | Not /really/ [fn:5]                 | No                  | Yes            | This is very popular but from a security perspective is just a hot mess.            |
...
[fn:5] MEW apparently works very well with a Trezor or similar
hardware-based cold wallet. A lot of people would refer to the Trezor
device as an "air-gapped" system since it stores your private keys in
a very secure way. However, I'm splitting hairs here and saying *no*
because the Trezor has to be plugged into a system that's on a network
to work. Potato po-tah-to.
