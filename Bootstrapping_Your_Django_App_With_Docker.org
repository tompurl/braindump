...
#+BEGIN_SRC sh
tee <<EOF > $HOME/Dev/Python/fun_with_docker_project/fun_with_docker/Dockerfile-dev-app
...
The =Dockerfile-dev-app= is a manifest that file will create the
...
<<cd_to_django_dir>>
sudo docker build -f Dockerfile-dev-app -t tompurl/fwd-app-dev . 
#+END_SRC
...
...
(28/29) Installing postgresql-libs (9.5.6-r0)
(29/29) Installing postgresql-dev (9.5.6-r0)
Executing busybox-1.24.2-r12.trigger
OK: 199 MiB in 60 packages
 ---> 51282dd2bbe1
Removing intermediate container 51874e54ae44
Step 5/8 : RUN rm -rf /var/cache/apk/*
 ---> Running in 2bb41724b1aa
 ---> 98f99ed3f2f1
Removing intermediate container 2bb41724b1aa
Step 6/8 : RUN pip install Django psycopg2
 ---> Running in eca91ccd3d8b
Collecting Django
...
Successfully installed Django-1.10.6 psycopg2-2.7.1
 ---> 9f6b3bd0b9df
Removing intermediate container eca91ccd3d8b
Step 7/8 : WORKDIR /usr/src/app
 ---> 7e542983f6ff
Removing intermediate container ed04c8325976
Step 8/8 : EXPOSE 8000
 ---> Running in b995424f83d3
 ---> 101f4c5ef8fb
Removing intermediate container b995424f83d3
Successfully built 101f4c5ef8fb
#+end_example
...
: tompurl/fwd-app-dev   latest              101f4c5ef8fb        5 minutes ago       286 MB
...
- =-p=: This command maps port 8000 on the host to port 8000 in the
  container, which we export in the =Dockerfile-dev-app= file above.
...
Ok, just for fun let's now ext the container by typing the =exit=
...
database, but that doesn't mean that we can run a super simple test. But
first, let's run our Dev server in a container:
...
#+RESULTS:
: 1770a52e39250a2c31101d5931cd0811192c4ff5511e3b474f8a5352e9f78432
...
: 1770a52e3925        tompurl/fwd-app-dev   "ash -c 'python ma..."   5 minutes ago       Up 5 minutes        0.0.0.0:8000->8000/tcp   fwd-app-dev
...
#+RESULTS:
: [23/Mar/2017 14:51:18] "GET / HTTP/1.1" 200 1767
...
#+RESULTS:
: a374e45e327c380cf86a98fc2537003ae9c96329e11d222d3a2dfe6fac820d39
...
waiting for server to start....LOG:  database system was shut down at 2017-03-23 17:29:39 UTC
...
LOG:  database system was shut down at 2017-03-23 17:30:14 UTC
LOG:  MultiXact member wraparound protections are now enabled
LOG:  database system is ready to accept connections
LOG:  autovacuum launcher started
#+end_example
...
#+RESULTS:
: a374e45e327c        postgres:9.6-alpine        "docker-entrypoint..."   21 minutes ago      Exited (0) 2 minutes ago                       fwd-pg-dev
...
#+RESULTS:
: 0ba6282b921cce40269c16178d77b4da8bff325f53c56e70580acd1fd80de19a
...
[
    {
        ...
        "Id": "0ba6282b921cce40269c16178d77b4da8bff325f53c56e70580acd1fd80de19a",
        "Created": "2017-03-23T19:02:18.372979564-05:00",
        "Scope": "local",
        "Driver": "bridge",
        "EnableIPv6": false,
        "IPAM": {
...
[
    {
        ...
        "Containers": {
            "a374e45e327c380cf86a98fc2537003ae9c96329e11d222d3a2dfe6fac820d39": {
                "Name": "fwd-pg-dev",
                "EndpointID": "09b09177451a2d34b5adf91c6142c62436a54ae59a7b2925b8edd032d146cb32",
...
#+RESULTS:
: 43bd84aafa790433f785b6c10366df7d5fb2ea11eb71516a6517f1da93775a07
