#+TITLE: Bootstrapping Your Django/Rails/Whatever App With Docker

#+BEGIN_COMMENT
Here's what I want to add:

- [X] Emphasis that while I'm using Docker here, the basic steps can be
  used to develop with any framework or language
- [X] Emphasis that this isn't the type of container that you push to a 
  public server - it's more of a lightweight VM
- [X] Add build prerequisites
#+END_COMMENT

I recently created a fairly simple web site for a non-profit.  Since I
just finished reading [[https://www.dockerbook.com/][The Docker Book]] [fn:1], I thought it would be nice to
...
on I wanted to make something good, not just sprinkle in enough Docker
to pad my resume. This wasn't simply a fun project that I could throw
away if things got weird after all. I therefore not only had to learn
how to make things work for five minutes with Docker - I had to learn
how to make something that was supportable, reliable and easy to
maintain.

I'm happy to say that I'm pretty happy with my results and I can't
wait to share my experiences.  My first lesson was in setting up my
development tools using Docker containers.

* Software Prerequisites
  :PROPERTIES:
  :CUSTOM_ID: softwareprereuisites
  :header-args: :exports both :results verbatim :eval never-export
  :END:

Don't you hate it when a tutorial makes you install a bunch of
weird-ass tools? Me too. So I'll try to keep it as simple as possible.

All of the scripts below were tested on a Debian 8 machine. Here's my
Docker version:
...
All you *absolutely need* to run the examples below is a Docker version
>= 1.9.  If you're using Windows or OS X you may need to also run the
examples from within some type of Linux-based VM. I can't provide
instructions on how to do that however because I don't have any
experience working with Docker on those platforms. If you run into any
issues or have any tips then I would [[file:index.org][love to hear]] about them!

There are a few commands below that require non-standard tools like
=curl= or =elinks=. I provide alternatives to these scripts when they
are used.

* Wait But Why Use Docker For Development?
  :PROPERTIES:
  :CUSTOM_ID: waitbutwhy
  :END:
...
But what I'm talking about at this phase is using a Docker container
as a *lightweight VM* to host everything I need *develop* my
application, and by that I mean *everything* that I need to:
...
The example in this article builds a development environment for a
Django-based application, but using the basic steps below you could
build a development environment for virtually /any/ type of
application (e.g. Rails, Node, JEE, etc.). 
...
- Most Python devs would also recommend that you use =virtualenv= to
  ...
  version or the Python 3 version?
...
* Iteration 1 - Starting Our Project
  :PROPERTIES:
  :header-args: :exports both :results verbatim :eval never-export
  :CUSTOM_ID: iteration1
  :END:
** Creating Our Dev Docker Image
   :PROPERTIES:
   :CUSTOM_ID: creatingourdevdockerimage
   :END:
...
For the remainder of this tutorial, this will be the base folder for
our Django project. If you would like to use a different folder then
don't forget to update the values in all of the code snippets below.
#+END_NOTE
...
Here's the results. I truncated them a little bit for brevity's sake.
Also, your results an be fairly different. The key is that the last
line tells you that it was "Successfully built".
...
** Running Our Dev Container
  :PROPERTIES:
  :CUSTOM_ID: runningourdevcontainer
  :END:      
...
If you don't have the =tree= application installed on your computer
then you can also view the files in your favorite file explorer.
...
Here's the results, which is just a hash for your container. Your
...
I tested this with =elinks=, which is a console-based web browser. You
can also test this with a regular web browser if you'd like, but
remember, I'm lazy so I like to use scripts :-)
...
Well, you could look at the logs for the =runserver= process if you
run this command from your Docker host (i.e. *not* from within the
container):
...
What this command does is read the output of the command that you 
specified when you started your container. In our case this is that
command:
...
Please note that this command should not be backgrounded with an =&=
or nohup command. If you did that then you wouldn't be able to read
the logs using the =docker logs= command.
...
* Iteration 2 - Networking And A Proper Database
  :PROPERTIES:
  :header-args: :exports both :results verbatim :eval never-export
  :CUSTOM_ID: iteration2
  :END:
** Creating A Database Image 
   :PROPERTIES:
   :CUSTOM_ID: creatingadatabaseimage
   :END:
...
** Networking Our Containers
   :PROPERTIES:
   :CUSTOM_ID: networkingourcontainers
   :END:
...
This commands returns the JSON representation of your network, and it
...
Earlier in the document I mentioned that it would be nice to be able
to use a *single skillset* for setting up development environments on
our laptops. We could then invest our precious time into learning how
to use and improve those skills instead of learning how to install the
latest cool kid language/framework/build tool combination quickly and
safely.

I'm confident that I could use the basic process above to set up a
development environment for many different frameworks quickly and
easily.  I therefore feel that my time invested in learning about
Docker was worthwhile. The basic process above has become my preferred
method for setting up everything I need to start working on a new
application.
...
[fn:1] I /highly/ recommend this book as an intro to Docker. It's by
far the best book that I've found on the topic and it's dirt cheap.
